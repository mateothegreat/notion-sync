...
export class ProgressService {
  ...
  constructor(private eventPublisher: (event: any) => Promise<void>) {}
  ...
  async startSection(exportId: string, sectionName: string, totalItems: number): Promise<void> {
    ...
    // Publish event
    await this.eventPublisher(ProgressEvents.sectionStarted(exportId, sectionName, totalItems));
  }

  async updateSectionProgress(exportId: string, sectionName: string, processedItems: number): Promise<void> {
    const tracker = this.getTracker(exportId);
  ...
  async recordItemProcessed(
    ...
    // Publish event
    await this.eventPublisher(ProgressEvents.itemProcessed(exportId, itemId, itemType, duration, success, error));
  }
  ...
  getProgress(exportId: string): ProgressInfo {
    ...
    const percentage = tracker.totalItems > 0 ? (tracker.processedItems / tracker.totalItems) * 100 : 0;
  ...
  getStatistics(exportId: string): ProgressStatistics {
    ...
    const completedSections = Array.from(tracker.sections.values()).filter((section) => section.endTime);

    const totalDuration = Date.now() - tracker.startTime.getTime();
    const averageItemTime = tracker.processedItems > 0 ? totalDuration / tracker.processedItems : 0;

    const errorRate = tracker.totalItems > 0 ? tracker.errors.length / tracker.totalItems : 0;
