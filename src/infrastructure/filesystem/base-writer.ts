...
import { FileMetadata, FileSystemConfig, FileWriter, FileWriteResult, ValidationResult } from "./types";
...
export abstract class BaseFileWriter implements FileWriter {
  ...
  async writeRawData(data: any, outputPath: string): Promise<FileWriteResult> {
    ...
    try {
      ...
      if (this.eventPublisher) {
        await this.eventPublisher(FileSystemEvents.fileCreated(finalPath, fileSize, this.getMimeType()));
      }
  ...
  protected async ensureDirectoryExists(dirPath: string): Promise<void> {
    try {
      ...
      if (this.eventPublisher) {
        await this.eventPublisher(FileSystemEvents.directoryCreated(dirPath));
      }
    }
  }
