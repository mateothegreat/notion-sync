...
import { NotionBlock, NotionConfig, NotionDatabase, NotionPage, RateLimitInfo } from "../../shared/types/index";
...
export class NotionClient implements NotionApiClient {
  ...
  async getPage(pageId: string): Promise<NotionPage> {
    return this.executeWithProtection("getPage", async () => {
  ...
  async getUsers(): Promise<any[]> {
    return this.executeWithProtection("getUsers", async () => {
      const startTime = Date.now();
  ...
  async search(query: string, options: any = {}): Promise<any> {
    return this.executeWithProtection("search", async () => {
      const startTime = Date.now();
  ...
  private async handleRateLimitError(error: any): Promise<void> {
    ...
    await this.eventPublisher(NotionEvents.rateLimitHit(0, this.rateLimitInfo.resetTime, retryAfter));
  ...
  private async handleApiError(error: any, operation: string, objectId: string): Promise<void> {
    ...
    await this.eventPublisher(NotionEvents.apiError(errorInfo, operation, 1));
  }
  ...
  private async publishObjectFetched(
    objectId: string,
    objectType: string,
    size: number,
    duration: number
  ): Promise<void> {
    await this.eventPublisher(NotionEvents.objectFetched(objectId, objectType, size, duration));
  }
}
